  ...
  for key in group.keys():
    ...
    for i in range(len(group[key])):
      ...
      #没找到
        ...
        averageinfo.append("未找到{0}小组的成员{1}的成绩，请确认名称是否正确".format(key,member))
      ...
      #计算能找到的
        ...
        if type(score[member][0]) == str:
          ...
          averageinfo.append("{0}的成绩内容是{1}，无法计入总分，将不计算此人".format(member,score[member][0]))
    ...
    averageinfo.append("{0}的总分为{1}，平均分为{2}".format(key,totalscore,averagescore))
...
def getentry():
  ...
  average = getgroupaverage(groupinf,scoreinf)
...
if __name__ == "__main__" :
  ...
  #设置标题
  window.title('Teachertool for Momoka -v 1.00.1')


  window.iconbitmap(os.getcwd()+'\\logo.ico')
  #窗口大小
  ...
  #小组框
  e1 = tk.Entry(window,textvariable=v1,width=80)
  e1.grid(row=0,column=1,padx=20,pady=10)

  #成绩框
  e2 = tk.Entry(window,textvariable=v2,width=80)
  e2.grid(row=1,column=1,padx=20,pady=10)
  
  #设置列表框
  scrollbar = tk.Scrollbar(window,orient="vertical")

  lb = tk.Listbox(window,width=100,height=25,bd=10)
